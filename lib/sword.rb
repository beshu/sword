...
def r;@r||={};end;def x;@x||=R::Builder.new;end;def w;@r.clear;@x.instance_eval{
...
write instance_eval(&l ?l:proc{|e|@@t<2?e.instance_eval{@@t+=1;@a[P]="#{@a[P].
chomp'/'}/index.html";@c=call(@a)[-1];@@t=0}:((@@t=0;raise@@p='NOT FOUND')if(
@@p||=0)==@@p=@a['REQUEST_PATH'];Sword.w;@c.redirect@@p)});@c.finish end end
