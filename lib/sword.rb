...
}"){c[m,self];t.new(f).render}:s<<"/#{f}"};use Rack::Static,:urls=>s;get(i=
'/favicon'<<z='.ico'){c[o[z],self];File.read"#{File.dirname __FILE__}#{i}"}
get('*'){(raise@@p='NOT FOUND')if(@@p||=0)==@@p=env['REQUEST_PATH'];(k=self.
